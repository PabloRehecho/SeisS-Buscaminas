...
public class Tablero extends Observable {
	...
	private int calcularMinas() {
		int sol = Buscaminas.getBuscaminas().calcularMinas(nivel);
	...
	private void anadirVecinosDD(int pFila, int pCol) {
		if (pFila == 0 && pCol != 0) {
			...
			((matriz[pFila][pCol])).anadirVecino((pFila + 1) + "," + (pCol - 1));
		} else if ((pFila == 0 && pCol == 0) || (pFila == filas && pCol == 0) || (pFila == 0 && pCol == columnas)
				|| (pFila == filas && pCol == columnas)) {
	...
	public void mostrarTablero() {
		...
		int fila;
		Casilla casilla;
		col = this.separarCoordenadasCol(this.separarCoordenadasString(MReset));
		fila = this.separarCoordenadasFil(this.separarCoordenadasString(MReset));
		casilla = buscarCasilla(fila, col);
		while (itr.hasNext()) {
			if (!casilla.estaDesvelada() && !casilla.tieneBandera() && MReset != null) {
				casilla.descubrir();
				setChanged();
				MReset = null;
	...
	/************************************************************
	 * Devuelve la casilla vacia en caso de que se encuentre * en la lista
	 * lCasillasVacias *
	 * 
	 * @param pFila *
	 * @param pCol *
	...
	private void revelarMinas() {
		...
		while (i > 0) {
			...
			} else if (!casilla.estaDesvelada() && casilla instanceof CasillaMinaReset) {
				casilla.descubrir();
				setChanged();
				notifyObservers(fila + "," + col + "," + 15);
	...
	public void descubrirCasilla(int pFila, int pCol) {
		...
		if (casilla instanceof CasillaMinaReset && !casilla.estaDesvelada() && !casilla.tieneBandera()) {
			casilla.descubrir();
			setChanged();
			notifyObservers(pFila + "," + pCol + "," + 12);

		} else if (casilla instanceof CasillaMina50 && !casilla.estaDesvelada() && !casilla.tieneBandera()) {
			this.revelarMinas();
			casilla.descubrir();
			setChanged();

			notifyObservers(pFila + "," + pCol + "," + 13);

		} else if (casilla instanceof CasillaMina && !casilla.estaDesvelada() && !casilla.tieneBandera()) {
			casilla.descubrir();
			setChanged();
	...
	/****************************************************
	 * Coge las coordenadas de la casilla y lo separa * metiendolo en un array de
	 * Strings -> String[] *
	 * 
	 * @param pCasilla *
